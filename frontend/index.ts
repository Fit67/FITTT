...
export const productService = {
  ...
  // Admin
  adminGetAll: (params: { page?: number; limit?: number; search?: string; status?: string } = {}): Promise<PaginatedResponse<Product>> =>
  ...
  toggle: (id: string) =>
    api(() => apiClient.patch<{ success:true; data: { id: string; status: string } }>(`/admin/products/${id}/toggle`)),
...
export const paymentService = {
  createPaymentIntent: (orderId: string, currency = 'usd') =>
    api(() => apiClient.post<{ success:true; data:{ clientSecret:string } }>(
      '/payments/create-intent', { orderId, currency },
    )),
}
